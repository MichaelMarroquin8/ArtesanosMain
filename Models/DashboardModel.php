...
class DashboardModel extends Mysql
{
	...
	{
		$sql = "SELECT COUNT(*) as total FROM persona WHERE status != 0 AND rolid = " . RARTESANO;
		$request = $this->select($sql);
		$total = $request['total'];
		return $total;
	}
	...
	{
		// Sanitize and escape the $idUser to prevent SQL injection
		$idUser = $_SESSION['userData']['idpersona'];
		$rolid = $_SESSION['userData']['idrol'];
		$where = "";
		// Construct the SQL query with the sanitized $idUser
		if ($rolid == RARTESANO) {
			$where = " WHERE status != 0 and personaid = " . $idUser;
		}
		...
		// Check if the request returned a result
		if ($request) {
			$total = $request['total'];
			return $total;
		} else {
			// Handle the case where the query did not return a result
			return 0; // or another appropriate value or error handling
		}
	}
